...
const requestCars = async () => {
    try {
        const { data: searchedCars } = await axios.get(
            'http://localhost:3000/searchCarList',
        );
        return searchedCars;
    } catch (error) {
        console.error(error);
...
const search = async (sortBy = 'price') => {
    ...
    const sortedCars = arraySort(searchedCars, (first, second) => {
        if (sortBy === 'price') {
            return (
                first.salefee +
                first.bohum[0].saleFee -
                (second.salefee + second.bohum[0].saleFee)
            );
        }
    ...
    if (sortBy === 'price') {
        sortedCars.forEach(s =>
            console.log('totalFee:', s.salefee + s.bohum[0].saleFee),
        );
    } else {
